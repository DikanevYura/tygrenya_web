...
const Header = () => {
  ...
  const handleScroll = () => {
    ...
    if (headerRef.current) {
      setIsScrolled(currentScrollY > headerRef.current.offsetHeight);
    }
  };
  ...
  return (
    <header
      className={cn('fixed inset-x-0 top-0 z-50 bg-[#fde1b1]', isOpenMenu && 'bg-[#fde1b1] h-svh')}
      ref={headerRef}
    >
      {isOpenMenu && (
        <>
      ...
      <div className="relative flex w-full flex-wrap items-center justify-between xl:flex-nowrap xl:px-12 xl:py-6">
        ...
        <div className={cn('hidden grow items-center justify-between xl:flex w-full mt-11 xl:mt-0 flex-col-reverse xl:flex-row px-4 py-5 xl:p-0 max-w-[350px] xl:max-w-full mx-auto xl:mx-0', isOpenMenu && 'flex p-0')}>
              ...
              {menuItems.map(item => (
                <li key={item.name} className="w-full xl:w-auto">
                  <button
                    ...
                    className={cn('flex xl:rounded-full px-4 xl:py-[7px] py-3 xl:text-sm text-base text-text-tugrenya-black xl:font-semibold hover:text-white duration-default hover:bg-[#5c3737]/50 justify-between')}
                  >
                    {item.name}
